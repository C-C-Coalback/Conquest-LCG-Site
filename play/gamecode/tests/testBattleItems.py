...
class BattleItemsTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_bodyguard_two_bodyguard_different_unit_enough_damage(self):
        ...
        test_game.p2.add_card_to_planet(card, 0)
        test_game.p2.attach_card(test_game.preloaded_find_card("Bodyguard"), 0, 1)
        test_game.p2.attach_card(test_game.preloaded_find_card("Bodyguard"), 0, 2)
        await test_game.update_game_event("P1", ["IN_PLAY", "1", "0", "1"])
        await test_game.update_game_event("P1", ["IN_PLAY", "2", "0", "0"])
        self.assertEqual(test_game.p2.get_damage_given_pos(0, 0), 1)
        self.assertEqual(test_game.p2.get_damage_given_pos(0, 1), 1)
    ...
    async def test_bodyguard_two_bodyguard_different_unit_not_enough_damage(self):
        ...
        await test_game.update_game_event("P1", ["IN_PLAY", "1", "0", "1"])
        await test_game.update_game_event("P1", ["IN_PLAY", "2", "0", "0"])
        self.assertEqual(test_game.last_automated_data_string, "GAME_INFO/AUTOMATED_DATA/Bodyguard/P2/|||IN_PLAY/2/0/1|||IN_PLAY/2/0/2|||")
        self.assertEqual(test_game.manual_bodyguard_resolution, True)
        self.assertEqual(len(test_game.stored_damage), 0)
        await test_game.update_game_event("P2", ["IN_PLAY", "2", "0", "1"])
        self.assertEqual(len(test_game.stored_damage), 1)
        self.assertEqual(test_game.p2.get_damage_given_pos(0, 0), 0)
        self.assertEqual(test_game.p2.get_damage_given_pos(0, 1), 1)
        self.assertEqual(test_game.p2.get_damage_given_pos(0, 2), 0)
    ...
    async def test_black_heart_ravager(self):
        ...
        await test_game.update_game_event("P1", ["pass-P1"])
        await test_game.update_game_event("P2", ["pass-P1"])
        card = test_game.preloaded_find_card("Black Heart Ravager")
        test_game.p1.add_card_to_planet(card, 0)
        test_game.p2.add_card_to_planet(card, 0)
        await test_game.update_game_event("P1", ["IN_PLAY", "1", "0", "1"])
        await test_game.update_game_event("P1", ["IN_PLAY", "2", "0", "1"])
        test_game.p2.cards = ["Raid"]
        test_game.p1.cards = []
        await test_game.update_game_event("P2", ["HAND", "2", "0"])
        ...
        test_game.p1.ready_given_pos(0, 1)
        self.assertEqual(test_game.p1.get_ready_given_pos(0, 1), True)
        print(test_game.player_with_combat_turn)
        await test_game.update_game_event("P1", ["IN_PLAY", "1", "0", "1"])
        self.assertEqual(test_game.p1.get_ready_given_pos(0, 1), False)
        await test_game.update_game_event("P1", ["IN_PLAY", "2", "0", "1"])
        self.assertEqual(len(test_game.stored_damage), 1)
    ...
    async def test_banshee_power_sword(self):
        ...
        await test_game.update_game_event("P1", ["pass-P1"])
        await test_game.update_game_event("P2", ["pass-P1"])
        test_game.p1.cards = ["Rogue Trader", "Rogue Trader", "Rogue Trader"]
        ...
        await test_game.update_game_event("P1", ["HAND", "1", "0"])
        await test_game.update_game_event("P1", ["pass-P1"])
        self.assertEqual(test_game.banshee_power_sword_extra_attack, 2)
        await test_game.update_game_event("P1", ["IN_PLAY", "2", "0", "1"])
        self.assertEqual(test_game.p2.get_damage_given_pos(0, 1), 3)
    ...
    async def test_fire_warrior_elite(self):
        ...
        await test_game.update_game_event("P1", ["pass-P1"])
        await test_game.update_game_event("P2", ["pass-P1"])
        test_game.p2.add_card_to_planet(test_game.preloaded_find_card("Fire Warrior Elite"), 0)
        test_game.p1.add_card_to_planet(test_game.preloaded_find_card("Fire Warrior Elite"), 0)
        await test_game.update_game_event("P1", ["IN_PLAY", "1", "0", "1"])
        await test_game.update_game_event("P1", ["IN_PLAY", "2", "0", "0"])
        await test_game.update_game_event("P2", ["CHOICE", "0"])
        await test_game.update_game_event("P2", ["IN_PLAY", "2", "0", "1"])
    ...
    async def test_repulsor_impact_field(self):
        ...
        test_game.p2.add_card_to_planet(test_game.preloaded_find_card("Fire Warrior Elite"), 0)
        test_game.p1.add_card_to_planet(test_game.preloaded_find_card("Fire Warrior Elite"), 0)
        test_game.p2.attach_card(test_game.preloaded_find_card("Repulsor Impact Field"), 0, 1)
        await test_game.update_game_event("P1", ["IN_PLAY", "1", "0", "1"])
        await test_game.update_game_event("P1", ["IN_PLAY", "2", "0", "1"])
        await test_game.update_game_event("P2", ["pass-P1"])
        await test_game.update_game_event("P2", ["CHOICE", "0"])
        self.assertEqual(test_game.p1.get_damage_given_pos(0, 1), 2)
    ...
    async def test_silvered_blade_avengers(self):
        ...
        await test_game.update_game_event("P2", ["pass-P1"])
        test_game.p2.cards = ["Rogue Trader"]
        test_game.p1.add_card_to_planet(test_game.preloaded_find_card("Silvered Blade Avengers"), 0)
        test_game.p2.add_card_to_planet(test_game.preloaded_find_card("Silvered Blade Avengers"), 0)
        await test_game.update_game_event("P1", ["IN_PLAY", "1", "0", "1"])
        await test_game.update_game_event("P1", ["IN_PLAY", "2", "0", "1"])
